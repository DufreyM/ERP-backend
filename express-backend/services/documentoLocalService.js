...
const { crearNotificacionesDeVencimientoDocumento } = require('./notificacionesVencimiento');
...
router.get('/', checkPermission('ver_documentos_locales'), async (req, res) => {
  try {
    ...
    const docs = await DocumentoLocal
      ...
      .whereNull('deletedat')
      .modify(query => {
        if (localId) query.where('local_id', localId);
      })
      .withGraphFetched('[usuario, local]')
      .modifyGraph('usuario', b => b.select('id', 'rol_id', 'nombre', 'apellidos', 'email'))
      .modifyGraph('local', b => b.select('id', 'nombre', 'direccion', 'nit_emisor'));
    res.json(docs);
  } catch (err) {
...
router.get('/:id', checkPermission('ver_documentos_locales'), async (req, res) => {
  try {
    const doc = await DocumentoLocal.query()
      .findById(req.params.id)
      .whereNull('deletedat')
      .withGraphFetched('[usuario, local]')
      .modifyGraph('usuario', b => b.select('id', 'nombre', 'apellidos', 'email'))
      .modifyGraph('local', b => b.select('id', 'nombre', 'direccion', 'nit_emisor'));
    if (!doc) return res.status(404).json({ error: 'Documento no encontrado' });
    res.json(doc);
  } catch (err) {
    res.status(500).json({ error: 'Error al obtener documento', details: err.message });
  }
});

// Crear nuevo documento
router.post('/', checkPermission('subir_documentos_locales'), async (req, res) => {
  try {
    let archivoURL = null;
    ...
    } else if (req.body.archivo) archivoURL = req.body.archivo;
    ...
    if (req.body.vencimiento) {
      await crearNotificacionesDeVencimientoDocumento(nuevo, { id: req.body.usuario_id }, req.body.local_id);
    }

    res.status(201).json(nuevo);
  } catch (err) {
    res.status(400).json({ error: 'Error al crear documento', details: err.message });
  }
});

// Actualizar documento existente
router.put('/:id', checkPermission('subir_documentos_locales'), async (req, res) => {
  try {
    ...
    if (archivoURL !== undefined) data.archivo = archivoURL;
    ...
    if (req.body.vencimiento) {
      await crearNotificacionesDeVencimientoDocumento(actualizado, { id: actualizado.usuario_id }, actualizado.local_id);
    }

    res.json(actualizado);
  } catch (err) {
    res.status(400).json({ error: 'Error al actualizar documento', details: err.message });
  }
});

// Eliminar documento (lógicamente)
router.delete('/:id', checkPermission('eliminar_documento_local'), async (req, res) => {
  try {
    const eliminado = await DocumentoLocal.query()
      .patchAndFetchById(req.params.id, { deletedat: new Date().toISOString() });
