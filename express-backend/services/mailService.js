...
async function sendVerificationEmail(to, token) {
    const link = `${process.env.BACKEND_API}/auth/verify?token=${token}&email=${encodeURIComponent(to)}`;
...
async function sendChangePassEmail(to, token) {
    const link = `${process.env.BACKEND_API}/auth/verify-reset?token=${token}&email=${encodeURIComponent(to)}`;
